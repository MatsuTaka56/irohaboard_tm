...
<div class="admin-contents-questions-index">
	...
	<div class="buttons_container">
		<button type="button" class="btn btn-primary btn-export" onclick="location.href='<?= Router::url(['action' => 'export', $content['Content']['id']]) ?>'"
			 title="本テストコンテンツに関連するテストコンテンツ情報、テスト問題情報をCSVファイルに出力し、、&#13;&#10;関係ファイルをZIPファイルにまとめてダウンロードする">エクスポート</button>
		<button type="button" class="btn btn-primary btn-import" onclick="location.href='<?= Router::url(['action' => 'import', $content['Content']['id']]) ?>'"
			 title="本テストコンテンツに対し、CSVファイルに定義したテスト問題情報、&#13;&#10;およびZIPファイルにまとめた関連ファイルを追加、またはすべて置き換えを行う">インポート</button>
		<button type="button" class="btn btn-primary btn-add"    onclick="location.href='<?= Router::url(['action' => 'add',    $content['Content']['id']]) ?>'"
			 title="本テストコンテンツに新たなテスト問題情報、関連ファイルを追加する">+ 追加</button>
	</div>
