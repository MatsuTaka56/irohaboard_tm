...
<?= $this->Html->link(__('<< 戻る'), ['action' => 'index', $course_id]) ?>
	<div class="panel panel-default">
		...
		<div class="panel-body">
			<li>コンテンツ情報、テスト問題情報が格納されたCSVファイルを選択し、インポートを行って下さい。</li>
			<li>CSVファイルの文字コードは「Shift-JIS」を使用してください。(UTF-8は使用できません。)</li>
			<li>行頭の「/*」から行末の「*/]はコメントとなります。</li>
			<li>各行は以下のフォームで指定しますが、１：コース情報は無視され、２：コンテンツ情報、３：テストコンテンツ情報、４：テスト問題情報がインポートされます。</li>
...
管理者権限でログインした場合のみ表示されます。（規定値：「公開」）">ステータス</th>
			...
			<?php
				...
				echo $this->Form->create('Content', Configure::read('form_defaults2'));
				// インポートモード
				echo $this->Form->inputRadio('import_mode', [
						'label' => __('インポートモード'), 
						'options' => Configure::read('content_import_mode'), 
						'default' => 'a', 
						'div' => 'form-group',
						'before' => '<label class="col col-sm-2 control-label" style="text-align:left">'.__('インポートモード').'</label>'
						]);
				// CSVファイル
